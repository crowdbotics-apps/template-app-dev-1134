      ...
      if (type === 'emailVerify') {
        emailVerify.render(locals, (err, results) => { //eslint-disable-line
          ...
          const mailOptions = {
            ...
            subject: 'Verify your Account with CB TaxiApp', // Subject line
      ...
      if (type === 'register') {
        register.render(locals, (err, results) => { //eslint-disable-line
          ...
          const mailOptions = {
            from: details.username, // sender address
            to: userObj.email, // list of receivers
            subject: 'Your Account with CB TaxiApp is created', // Subject line
            text: results.text, // plain text body
            html: results.html // html body
          };
          transporter.sendMail(mailOptions, (error, info) => {
      ...
      if (type === 'endTrip') {
        endTrip.render(locals, (err, results) => {
          ...
          const mailOptions = {
            from: details.username, // sender address
            to: userObj.email, // list of receivers
            subject: 'Ride Details with CB TaxiApp', // Subject line
            text: results.text, // plain text body
            html: results.html // html body
          };
          transporter.sendMail(mailOptions, (error, info) => {
      ...
      if (type === 'forgot') {
        forgot.render(locals, (err, results) => {
          ...
          const mailOptions = {
            from: details.username, // sender address
            to: userObj.email, // list of receivers
            subject: 'Your Account Password with CB TaxiApp', // Subject line
            text: results.text, // plain text body
            html: results.html // html body
          };
          transporter.sendMail(mailOptions, (error, info) => {
      ...
      if (type === 'rideAccept') {
        rideAccept.render(locals, (err, results) => {
          ...
          const mailOptions = {
            from: details.username, // sender address
            to: userObj.email, // list of receivers
            subject: 'CB TaxiApp Driver Details', // Subject line
            text: results.text, // plain text body
            html: results.html // html body
          };
          transporter.sendMail(mailOptions, (error, info) => {
